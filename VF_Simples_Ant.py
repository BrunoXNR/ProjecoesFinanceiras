...
def valor_futuro_sem_aporte(capital_inicial, taxa_juros, prazo):
    patrimonio_mensal_sem_aporte = []
    
    for mes in range(1, prazo + 1):
        valor_futuro = capital_inicial * (1 + taxa_juros) ** mes
        patrimonio_mensal_sem_aporte.append(valor_futuro)
    
    return patrimonio_mensal_sem_aporte


# Chamada das Funções
valor_futuro_final_com_aporte, patrimonio_mensal = valor_futuro_com_aporte(capital_inicial, taxa_juros, prazo, aporte)
patrimonio_mensal_sem_aporte = valor_futuro_sem_aporte(capital_inicial, taxa_juros, prazo)

# Retornar o  valor final da projeção com aportes
print(f"Valor Futuro com Aporte: R$ {valor_futuro_final_com_aporte:.2f}")

# Calcular e retornar o valor final da projeção sem aportes
valor_futuro_final_sem_aporte = patrimonio_mensal_sem_aporte[-1]
print(f"Valor Futuro sem Aporte: R$ {valor_futuro_final_sem_aporte:.2f}")
...
df = pd.DataFrame({
    ...
    'Patrimônio sem Aporte': patrimonio_mensal_sem_aporte
})
...
plt.title('Patrimônio Projetado (Parcelas Antecipadas)', fontsize=16)
